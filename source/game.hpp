...
private:
    Board m_board;
    Shader m_notexture;
    Shader m_withtexture;
    Shader m_skybox;
    std::vector<Forklift> m_forklifts;
    std::unique_ptr<Box> m_boxLeft{nullptr};
    std::unique_ptr<Box> m_boxCenter{nullptr};
    std::unique_ptr<Box> m_boxRight{nullptr};
    unsigned int m_skyboxVAO, m_skyboxVBO;
    unsigned int m_cubemapTexture;
    void setupForklifts(const std::string& logFile);
};
